...
export class GitHubTokenListResolutionStrategy {
  repositories = ['https://raw.githubusercontent.com/solflare-wallet/token-list/master/solana-tokenlist.json']
  ...
  const responses: TokenList[] = (await Promise.all(
    files.map(async (repo) => {
      try {
        ...
        return require('@solflare-wallet/token-list/solana-tokenlist.json')
      }
    }),
  )) as TokenList[]
...
export class TokenListProvider {
  ...
  resolve = async (strategy: Strategy = Strategy.GitHub): Promise<TokenListContainer> => {
